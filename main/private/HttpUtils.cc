...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  //
  // (Note: It can be tricky to handle all of the cases where "."
  ...
  char root[PATH_MAX];
  char test[PATH_MAX];
  if (realpath(root_dir.c_str(), root) == NULL)
    return false;
  strcat(root, "/");
  if (realpath(test_file.c_str(), test) == NULL) {
    return false;
  }
  return strncmp(root, test, strlen(root)) == 0;
}
...
string EscapeHtml(const string& from) {
  ...
  // that need to be escaped for XML documents.
