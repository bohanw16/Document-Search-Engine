/*
 * Copyright Â©2022 Dylan Hartono.  All rights reserved.  Permission is
...
static void GetPortAndPath(int argc,
  ...
  *port = atoi(argv[1]);
  if (*port < 1025) {
    cerr << "port number should be >1025\n";
    Usage(argv[0]);
  }
  ...
  *path = argv[2];
  ...
  for (int i = 3; i < argc; i++) {
    stat(argv[i], &dirr);
    if (!S_ISREG(dirr.st_mode)) {
      cerr << "indices incorrect\n";
      Usage(argv[0]);
    }
    indices->push_back(argv[i]);
  }

  // check if we have at least 1 index
  if (indices->size() == 0) {
    cerr << "no index\n";
    Usage(argv[0]);
  }
